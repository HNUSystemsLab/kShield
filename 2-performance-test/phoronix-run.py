...
def main(argv):
    filename = argv[1]
    sh.sendline(b'phoronix-test-suite benchmark apache pts/build-linux-kernel system/wireguard pts/encode-mp3 pts/compress-xz system/gimp openssl osbench pts/glibc-bench perf-bench pts/redis pts/perl-benchmark')
    #sh.sendline(b'phoronix-test-suite benchmark osbench')
    ...
    print('glibc-bench')
    recvuntil_send(b'Benchmark', b'17', 20)
    ...
    text = sh.recvuntil(b'Enter a unique name to describe this test run / configuratio', timeout=20)
    print(text)
    print('\n\n\n')
    sh.sendline(b'xxxx')
